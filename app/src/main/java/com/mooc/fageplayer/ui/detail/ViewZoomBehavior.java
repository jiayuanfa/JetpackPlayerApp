...
public class ViewZoomBehavior extends CoordinatorLayout.Behavior<FullScreenPlayerView> {
     ...
     * 并全局保存下 child view
     * 并计算出初始的时候 child的底部值 也就是它的高度  我们后续拖拽滑动的时候，他就是最大高度的限制
     * 与此同时 还需要计算出，当前页面是否可以进行视频的全屏展示，即height > width 即可
     * @param parent
     * @param child
    ...
    private ViewDragHelper.Callback mCallback = new ViewDragHelper.Callback() {
        ...
        /**
         * 告诉ViewDragHelper 什么时候可以拦截手指触摸的这个View的手势分发
         * @param child
         * @param pointerId
         * @return
         */
        ...
        public int clampViewPositionVertical(@NonNull View child, int top, int dy) {
             ...
             */
            // 手指从下往上滑动。dy < 0 这意味着refchild的底部会被向上移动，所以它的底部的最小值不能小于minHeight, 也就是视频播放View最大的的缩放极限
            if (dy < 0 && refChild.getBottom() <= minHeight
                    // 手指从上往下滑动 dy > 0 意味着refchild的底部会被向下移动 所以他的底部的最大值 不能超过父容器的高度 也就是childOriginHeight
                || (dy > 0 && refChild.getBottom() >= childOriginalHeight)
                // 手指从屏幕上方往下滑动 如果scrollingView 还没有滑动到列表的最顶部
                // 也意味着列表还可以向下滑动 此时咱们应该让列表自行滑动 不做拦截
            ...
            if (dy > 0) {
                ...
                // 如果本次滑动的dy值 追加上 refchild的bottom值 会超过父容器的最大高度值
                // 此时 咱们应该计算一下
    ...
    private class FlingRunnable implements Runnable {
        ...
        /**
         * startX: 开始的X值，由于我们不需要在水平方向上滑动，所以为0
         * startY: 开始滑动的时候的Y值，那就是flingView的bottom值
         * xvel: 水平方向上的速度，实际上为0
         * yvel: 垂直方向上的速度。即松手的速度
         * minX: 水平方向上 滚动回弹的越界最小值，给0即可
         * maxX: 水平方向上 滚动回弹的越界最大值，实际上给0也是一样的
         * minY: 垂直方向上 滚动回弹的越界最小值 给0即可
         * maxY: 垂直方向上 滚动回弹的越界最大值，实际上给0也是一样的
         * @param xvel
         * @param yvel
         */
