...
public class FileUploadManager {
    ...
    static {
        ...
        configuration.setConnectionTimeout(15 * 1000); // 链接超时，默认15秒
        configuration.setSocketTimeout(15 * 1000); // socket超时 默认15秒
        configuration.setMaxConcurrentRequest(5); // 最大并发请求数 默认5个
        configuration.setMaxErrorRetry(2); // 失败后最大的重试次数 默认2次
        OSSLog.disableLog(); // 这个开启会支持写入手机sk卡中的一份日志文件位置在SDCard_path\OSSLog\logs.csv

        oss = new OSSClient(AppGlobals.getApplication(), END_POINT, credentialProvider, configuration);
    }
